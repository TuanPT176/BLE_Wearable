/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file    WEARABLE_app.h
  * @author  MCD Application Team
  * @brief   Header for WEARABLE_app.c
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2026 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef WEARABLE_APP_H
#define WEARABLE_APP_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
/* USER CODE BEGIN Includes */
#include "../../Application/wearable_data.h"
/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
typedef enum
{
  WEARABLE_CONN_HANDLE_EVT,
  WEARABLE_DISCON_HANDLE_EVT,

  /* USER CODE BEGIN Service1_OpcodeNotificationEvt_t */

  /* USER CODE END Service1_OpcodeNotificationEvt_t */

  WEARABLE_LAST_EVT,
} WEARABLE_APP_OpcodeNotificationEvt_t;

typedef struct
{
  WEARABLE_APP_OpcodeNotificationEvt_t          EvtOpcode;
  uint16_t                                 ConnectionHandle;

  /* USER CODE BEGIN WEARABLE_APP_ConnHandleNotEvt_t */

  /* USER CODE END WEARABLE_APP_ConnHandleNotEvt_t */
} WEARABLE_APP_ConnHandleNotEvt_t;
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */
/* USER CODE END EC */

/* External variables --------------------------------------------------------*/
/* USER CODE BEGIN EV */

/* USER CODE END EV */

/* Exported macros -----------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

/* Exported functions ------------------------------------------------------- */
void WEARABLE_APP_Init(void);
void WEARABLE_APP_EvtRx(WEARABLE_APP_ConnHandleNotEvt_t *p_Notification);
/* USER CODE BEGIN EF */
const uint8_t *WEARABLE_APP_GetLatestSensorData(uint16_t *length);
const uint8_t *WEARABLE_APP_GetLatestDeviceStatus(uint16_t *length);
void WEARABLE_APP_NotifyMotionInterruptFromISR(void);
/* USER CODE END EF */

#ifdef __cplusplus
}
#endif

#endif /*WEARABLE_APP_H */
